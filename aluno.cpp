...
template<typename T>
void Aluno<T>::setCursoAluno(string cpfAluno){
...
ostream& operator << (ostream& os, const Aluno<U>& alu){
	...
	os << "Média do Aluno:" << alu.media << endl;
...
istream& operator >> (istream& is, Aluno<U>& alu){
	...
	is >> alu.getNomeAluno() >> alu.getIdadeAluno() >> alu.getSexoAluno() >> alu.getTelefoneAluno() >> alu.getEmailAluno() >> alu.getCpfAluno() >> alu.getCurso();
